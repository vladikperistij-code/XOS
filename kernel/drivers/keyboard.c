...
char get_char_from_scancode(unsigned char scancode) {
    switch(scancode) {
        ...
        case 0x39: return ' ';  // Пробіл
        case 0x1C: return '\n'; // Enter
        case 0x0E: return '\b'; // Backspace
        default: return 0;      // Якщо код невідомий - ігноруємо
    }
}
...
void kinput(char* buffer) {
    int i = 0;
    char* debug_ptr = (char*)0xB8000 + 158; // Правий верхній кут екрана

    while (1) {
        unsigned char status = inb(0x64);
        
        // ВІЗУАЛЬНИЙ ДЕБАГ: виводимо статус контроллера в куту
        debug_ptr[0] = (status & 1) ? '1' : '0'; 
        debug_ptr[1] = 0x0C; // червоний колір

        if (!(status & 0x01)) continue; // чекаємо поки появиться біт данних
